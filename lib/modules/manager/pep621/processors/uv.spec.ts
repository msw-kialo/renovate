  ...
  describe('updateArtifacts()', () => {
      ...
      expect(execSnapshots).toMatchObject([
        ...
        },
        {
          cmd:
            ...
            '&& ' +
            'install-tool uv v2.5.0 ' +
            '&& ' +
            'uv lock --python-preference only-system --upgrade-package dep1' +
            '"',
        },
      ]);
    });
    ...
    it('return update dep update', async () => {
      ...
      const updatedDeps = [
        ...
        { depName: 'group2/dep6', depType: depTypes.uvDevDependencies },
        {
          depName: 'group3/dep7',
          depType: depTypes.uvDevDependencies,
        },
        { depName: 'group3/dep8', depType: depTypes.uvDevDependencies },
      ...
      expect(execSnapshots).toMatchObject([
        {
          cmd: 'uv update --no-sync --update-eager dep1 dep2',
        },
        {
          cmd: 'uv update --no-sync --update-eager -G group1 dep3 dep4',
        },
        {
          cmd: 'uv update --no-sync --update-eager -dG group2 dep5 dep6',
        },
        {
          cmd: 'uv update --no-sync --update-eager -dG group3 dep7 dep8',
        },
      ]);
    });
    ...
    it('return update on lockfileMaintenance', async () => {
      ...
      expect(execSnapshots).toMatchObject([
        {
          cmd: 'uv lock --python-preference only-system --upgrade',
          options: {
